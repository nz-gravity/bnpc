...
def brokenPower(amp:float,gam:float,psi:float,frequencies:np.ndarray)->np.ndarray:
...
def signal_density(b:float,g:float,psi:float,f:np.ndarray, signal_model:int=1)->np.ndarray:
    '''
    Signal density
    :param b: slope
    :param g: log amplitude
...
def signal_prior_sum(b:float,g:float,psi:float,signal_model:int=1)->float:
    '''
    signal prior sum
    :param b: slope
    :param g: log amplitude
