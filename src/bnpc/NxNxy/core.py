...
def psd(Snpar:np.ndarray, Spar:np.ndarray=1, modelnum:int=0)->np.ndarray:
    '''
...
def logdiffexp(a, b):
    ...
    if np.any(valid_mask):
        ...
        out[valid_mask] = (
                a[valid_mask] +
                np.log(1.0 - np.exp(b[valid_mask] - a[valid_mask]))
        )
...
def loglike(A:np.ndarray,E:np.ndarray,T:np.ndarray,S:np.ndarray,s_n:np.ndarray)->float:
...
def prior_sum(lamb_lpri:float, phi_lpri:float, delta_lpri:float, lamb_lpri_xy:float, phi_lpri_xy:float, delta_lpri_xy:float, sig_prior_sum:float)->float:
    return lamb_lpri + phi_lpri + delta_lpri + lamb_lpri_xy + phi_lpri_xy + delta_lpri_xy + sig_prior_sum
...
def llike_prisum_psd(obj, ind):
    obj.logpsplines_x.splines_mat[ind, :] = dens(lam=obj.logpsplines_x.lam_mat[ind, :],
                                                splines=obj.logpsplines_x.splines.T)  # Spline PSD of T channel
    obj.npsdNx[ind, :] = psd(Snpar=obj.logpsplines_x.splines_mat[ind, :], Spar=obj.Spar_x,
                            modelnum=obj.modelnum)  # Noise PSD of T channel
    obj.npsdNxy[ind, :] = psd(Snpar=obj.logpsplines_xy.splines_mat[ind, :], Spar=obj.Spar_xy,
                            modelnum=obj.modelnum)  # Noise PSD of T channel
    obj.npsdT[ind, :] = N_T (obj.npsdNx[ind, :], obj.npsdNxy[ind, :])  # Noise PSD of T channel

    obj.npsdA[ind, :] = N_A(obj.npsdNx[ind, :], obj.npsdNxy[ind, :])  # Noise PSD of T channel

    obj.totpsd[ind, :] = tot_psd(s_n=obj.npsdA[ind, :], s_s=obj.signal.s_s[ind, :])  # Total PSD

    obj.llike[ind] = loglike(A=obj.A, E=obj.E, T=obj.T, S=obj.totpsd[ind, :], s_n=obj.npsdT[ind, :])  # Log likelihood of T channel

    obj.prisum[ind]=prior_sum(lamb_lpri=lamb_lprior(obj.logpsplines_x.lam_mat[ind, :], obj.logpsplines_x.phi[ind], obj.logpsplines_x.P, obj.k),
                     phi_lpri=phi_lprior(obj.logpsplines_x.phi[ind], obj.logpsplines_x.delta[ind]),
                     delta_lpri=delta_lprior(obj.logpsplines_x.delta[ind]),lamb_lpri_xy=lamb_lprior(obj.logpsplines_xy.lam_mat[ind, :], obj.logpsplines_xy.phi[ind], obj.logpsplines_xy.P, obj.k),
                     phi_lpri_xy=phi_lprior(obj.logpsplines_xy.phi[ind], obj.logpsplines_xy.delta[ind]),
                     delta_lpri_xy=delta_lprior(obj.logpsplines_xy.delta[ind]),
                     sig_prior_sum=signal_prior_sum(obj.signal.b[ind], obj.signal.g[ind], obj.signal.psi[ind], obj.signal_model)) # Prior sum
