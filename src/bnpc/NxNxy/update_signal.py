import numpy as np
from .core import lpost, tot_psd, prior_sum, loglike, psd, dens, lamb_lprior, phi_lprior, delta_lprior, N_A, N_T
...
def S_and_prisum(self, b_val, g_val, psi_val, ind):
    ...
    npsdNx = psd(Snpar=self.logpsplines_x.splines_mat[ind, :], Spar=self.Spar_x,
                            modelnum=self.modelnum)  # Noise PSD of T channel
    npsdNxy = psd(Snpar=self.logpsplines_xy.splines_mat[ind, :], Spar=self.Spar_xy,
                            modelnum=self.modelnum)  # Noise PSD of T channel
    ...
    prisum = prior_sum(lamb_lpri=lamb_lprior(self.logpsplines_x.lam_mat[ind, :], self.logpsplines_x.phi[ind], self.logpsplines_x.P, self.k),
                     phi_lpri=phi_lprior(self.logpsplines_x.phi[ind], self.logpsplines_x.delta[ind]),
                     delta_lpri=delta_lprior(self.logpsplines_x.delta[ind]),lamb_lpri_xy=lamb_lprior(self.logpsplines_xy.lam_mat[ind, :], self.logpsplines_xy.phi[ind], self.logpsplines_xy.P, self.k),
                     phi_lpri_xy=phi_lprior(self.logpsplines_xy.phi[ind], self.logpsplines_xy.delta[ind]),
                     delta_lpri_xy=delta_lprior(self.logpsplines_xy.delta[ind]),
                     sig_prior_sum=signal_prior_sum(b_val, g_val, psi_val, self.signal_model))
...
def update_signal_param(self, ind):
            ...
            s_n,S, prisum = S_and_prisum(self, self.signal.b[ind - 1], self.signal.g[ind - 1], self.signal.psi[ind - 1], ind)
            ...
            s_n, S, prisum = S_and_prisum(self, self.signal.b[ind], self.signal.g[ind], self.signal.psi[ind], ind)

            ftheta_star = lpost(loglike(A=self.A, E=self.E, T=self.T, S=S, s_n=s_n), prisum)
