...
def loglike_AET(A:np.ndarray,E:np.ndarray,T:np.ndarray,S:np.ndarray,s_n:np.ndarray)->float:
...
def lamb_A_lprior(lam_A:np.ndarray, lam_mat:np.ndarray, P_A:np.ndarray, k:int)->float:
    '''
    ...
    mult = phi @ P_A @ phi
    prior = -k * np.log(determinant(mult)) / 2 - np.matmul(np.transpose(lam_A - mean_vec),
                                                           np.matmul(mult, (lam_A - mean_vec))) / 2
    if prior is np.inf:
        return -np.inf
    return prior
...
def prior_sum(lamb_lpri:float=0, phi_lpri:float=0, delta_lpri:float=0, lamb_lpri_A:float=0, sig_prior_sum:float=0)->float:
...
def llike_prisum_psd(obj, ind):
    obj.logpsplines_T.splines_mat[ind, :] = dens(lam=obj.logpsplines_T.lam_mat[ind, :],
                                                splines=obj.logpsplines_T.splines.T)  # Spline PSD of T channel
    obj.npsdT[ind, :] = psd(Snpar=obj.logpsplines_T.splines_mat[ind, :], Spar=obj.Spar,
                           modelnum=obj.modelnum)  # Noise PSD of T channel
    obj.llike[ind] = loglike(pdgrm=obj.T, S=obj.npsdT[ind, :])  # Log likelihood of T channel
    obj.prisum[ind]=prior_sum(lamb_lpri=lamb_lprior(obj.logpsplines_T.lam_mat[ind, :], obj.logpsplines_T.phi[ind], obj.logpsplines_T.P, obj.k),
                     phi_lpri=phi_lprior(obj.logpsplines_T.phi[ind], obj.logpsplines_T.delta[ind]),
                     delta_lpri=delta_lprior(obj.logpsplines_T.delta[ind])) # Prior sum of T channel
    if obj.A is not None:
        obj.logpsplines_A.splines_mat[ind, :] = dens(lam=obj.logpsplines_A.lam_mat[ind, :], splines=obj.logpsplines_A.splines.T)

        obj.npsdA[ind, :] = psd(Snpar=obj.logpsplines_A.splines_mat[ind, :], Spar=obj.Spar_A,
                               modelnum=obj.modelnum)  # Noise PSD of A channel
        ...
        obj.prisum[ind]+=prior_sum(lamb_lpri_A=lamb_A_lprior(obj.logpsplines_A.lam_mat[ind, :], obj.logpsplines_T.lam_mat[:ind + 2, :], obj.logpsplines_A.P, obj.k),
                          sig_prior_sum=signal_prior_sum(obj.signal.b[ind], obj.signal.g[ind], obj.signal.psi[ind], obj.signal_model))
