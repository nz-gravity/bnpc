...
from ..signal.utils import signal_density, signal_prior_sum
from .core import (
    delta_lprior,
    dens,
    lamb_A_lprior,
...
def l_p_T(self, lam: np.ndarray, j: int):
    ...
    """
    S = psd(
        dens(lam, self.logpsplines_T.splines.T),
        Spar=self.Spar,
        modelnum=self.modelnum,
    )  # noise PSD of T channel
    prisum = prior_sum(
        lamb_lprior(lam, self.logpsplines_T.phi[j], self.P, self.k),
...
def l_p_A(self, lam: np.ndarray, j: int):
    ...
    prisum = prior_sum(
        lamb_lpri_A=lamb_A_lprior(
            lam,
            self.logpsplines_T.lam_mat[: j + 2, :],
    ...
    noise_A = psd(
        dens(lam, self.logpsplines_A.splines.T),
        Spar=self.Spar_A,
        modelnum=self.modelnum,
...
def update_lambda_T(self, i: int):
    ...
    """
    self.logpsplines_T.sigma = sigmaupdate(
        self.logpsplines_T.accept_frac, self.logpsplines_T.sigma
    )
    (
        self.logpsplines_T.lam_mat[i, :],
        self.logpsplines_T.accept_frac,
    ) = lam_loop(self, loop_index=i - 1)
...
def lam_A_loop(self, i: int) -> np.ndarray:
    ...
    if (self.Uv_am[i] < 0.05) or (i <= 2 * self.k):
        ...
        lam_star = multivariate_normal.rvs(
            mean=lam,
            cov=self.logpsplines_A.c_amh * self.logpsplines_A.covObj,
            size=1,
        )
...
def update_lambda_A(self, i: int = 0):
    ...
    self.logpsplines_A.covObj = updateCov(
        self.logpsplines_A.lam_mat, self.logpsplines_A.covObj, i
    )
