...
from ..logSplines import LogSplineData, logPsplines, update_delta, update_phi
...
class Sampler:
    ...
    def MCMCloop(self):
        for i in range(1, self.n):
            ...
            # sample phi
            self.logpsplines_T.phi[i] = update_phi(
                self.logpsplines_T.lam_mat[i, :],
                None,
                self.logpsplines_T.delta[i - 1],
                self.logpsplines_T.a_phi,
            )

            # sample delta
            self.logpsplines_T.delta[i] = update_delta(
                self.logpsplines_T.phi[i], self.logpsplines_T.a_delta
            )
...
class MCMCResult:
    ...
    def basic_summary(self):
        ...
        if self.b is not None:
            ...
            print(
                f"log amplitude: mean={mp:.3f}, sd={sp:.3f}, 95% CI=({lp:.3f}, {up:.3f})"
            )

        if self.g is not None:
            md, sd, ld, ud = basic_summaries(self.g)
            print(
                f"slope: mean={md:.3f}, sd={sd:.3f}, 95% CI=({ld:.3f}, {ud:.3f})"
            )
