import numpy as np
from core import lpost, tot_psd, prior_sum, lamb_A_lprior, loglike_A, psd, dens
...
def S_and_prisum(self, b_val, g_val, psi_val, ind):
    ...
    noise_A = psd(dens(self.logpsplines_A.lam_mat[ind, :], self.logpsplines_A.splines.T), Spar=self.Spar_A, modelnum=self.modelnum) #noise PSD of A channel
    prisum = prior_sum(
        lamb_lpri_A=lamb_A_lprior(self.logpsplines_A.lam_mat[ind, :], self.logpsplines_T.lam_mat[:ind + 1, :], self.logpsplines_A.P, self.k),
...
def update_signal_param(self, ind):
            ...
            if self.signal_model == 2:
                ...
                self.b[ind] = np.random.normal(self.b[ind - 1], 0.1)
                self.g[ind] = np.random.normal(self.g[ind - 1], 0.1)
