...
def test_panelty_mat_non_singular():
    ...
    assert not np.isclose(det_p_mat, 0.0), "Penalty matrix is singular or nearly singular."

def test_panelty_mat_linear():
    """
    Test that linear penalty matrix is computed as expected.
    """
    knots = np.array([0, 1, 2])  # not used if linear=True, but you still must pass it
    d = 1
